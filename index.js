...
function handleEditItem(e) {
  ...
  document.getElementById("maintitle").innerText = "Ubah Tugas Anda Dibawah :";
  editItem = e.target;
...
function handleEditClick(e) {
  ...
  if (!editedItemText.trim()) {
    displayErrorMessage("Tambahkan Judul Tugas!!!");
    return false;
  }

  if (!editedItemText) {
    displayErrorMessage("Judul Tugas Jangan Kosong!!!");
    return false;
  }

  if (editedDueDate < new Date(currentDate)) {
    displayErrorMessage("Tanggal Tugas Sudah Terlewat!!!");
    return false;
  }

  if (!editedPriority) {
    displayErrorMessage("Tolong Pilih Prioritas!!!");
    return false;
  }
  ...
  displaySuccessMessage("Tugas Berhasil Diubah");
  editItem = null;
  itemInput.value = "";
  descriptionInput.value = "";
  dueDateInput.value = "";
  document.getElementById("maintitle").innerText = "Rencanakan Tugas Anda";
  ...
  function deleteTask(taskTitle) {
    ...
    if (taskElement) {
      taskElement.remove();
      saveTasksToLocalStorage();
      displaySuccessMessage(`Task "${taskTitle}" Berhasil Dihapus.`);
    } else {
  ...
  function editTask(
    ...
    if (taskElement) {
      ...
      saveTasksToLocalStorage();
      displaySuccessMessage(`Task "${oldTitle}" Berhasil Diubah.`);
    } else {
  ...
  function addTask(taskTitle, dueDate, priority) {
    ...
    if (taskExists) {
      displayErrorMessage("Tugas Telah Tersedia !!!");
      return;
    }
...
function showComfirmboxForDuplicateTasks() {
  ...
  delalert_title.innerHTML = "&#9888; Tugas Ini Telah Tersedia";
  delalert_title.className = "alert alert-danger";
  delalert_title.role = "Peringatan";
...
function addItem(e) {
  ...
  if (!newTaskTitle) {
    displayErrorMessage("Judul Tugas Harus di Isi!!!");
    tasksHeading.classList.add("hidden");
    searchBar.classList.add("hidden");
    return false;
  } else if (!dueDate) {
    displayErrorMessage("Atur Jadwal Tugas !!!");
    return false;
...
function handleItemClick(e) {
  if (e.target.classList.contains("delete")) {
    ...
    const handleYesClick = () => {
      ...
      tasksCheck();
      displaySuccessMessage("Tugas Berhasil Dihapus !!!");
      saveTasksToLocalStorage();
      confirmYesButton.removeEventListener("click", handleYesClick);
...
function clearAllTasks() {
  ...
  if (taskList.children.length > 0) {
    alertTitle.innerHTML = "&#9888; Ingin Menghapus Semua Tugas?";
